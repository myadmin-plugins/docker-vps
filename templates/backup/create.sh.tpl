...
if [ "$pool" = "zfs" ]; then
	...
	{$base}/vps_docker_lvmcreate.sh {$vzid} $size || exit
fi
iprogress 10 &
...
if /usr/bin/virsh dominfo {$vzid} >/dev/null 2>&1; then
	...
	if [ ! -e /usr/libexec/qemu-docker ] && [ -e /usr/bin/docker ]; then
	  sed s#"/usr/libexec/qemu-docker"#"/usr/bin/docker"#g -i {$vzid}.xml
	fi;
fi
if [ "{$module}" = "quickservers" ]; then
...
if [ ! -e /vz/templates/template.281311.qcow2 ]; then
  wget -O /vz/templates/template.281311.qcow2 http://dockertemplates.is.cc/cl/template.281311.qcow2
fi
...
if [ $error -eq 0 ]; then
	if [ "$adjust_partitions" = "1" ]; then
		...
		if [ "$fs" = "83" ]; then
			...
			php {$base}/vps_docker_password_manual.php {$rootpass} "/vz/mounts/{$vzid}$pn"
			if [ -e /vz/mounts/{$vzid}$pn/home/docker ]; then
				echo docker:{$rootpass} | chroot /vz/mounts/{$vzid}$pn chpasswd
			fi;
	...
	{$base}/tclimit $ip;
	if [ "{$clientip}" != "" ]; then
		{$base}/vps_docker_setup_vnc.sh {$vzid} {$clientip|escapeshellarg};
	fi;
	...
	/admin/dockerenable blocksmtp {$vzid}
	rm -f /tmp/_securexinetd;
	if [ "{$module}" = "vps" ]; then
		/admin/dockerenable ebflush;
